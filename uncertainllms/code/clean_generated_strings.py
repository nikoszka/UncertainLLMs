...
parser.add_argument('--generation_model', type=str, default='Llama-2-7b-hf')
...
generation_tokenizer = AutoTokenizer.from_pretrained(f"meta-llama/Llama-2-7b-hf", use_fast=False, cache_dir=config.data_dir)

wandb.init(project='nlg_uncertainty', id=args.run_id, config=args, resume='allow')

run_name = wandb.run.name

tokenizer = AutoTokenizer.from_pretrained(f"meta-llama/Llama-2-7b-hf", use_fast=False, cache_dir=config.data_dir)
